...
**1) IOCTL-시드 분리 및 가변 페이로드 제어 (양성/음성 검증 체계 구축)**

**IOCTL 제어 코드와 뮤테이션 시드(base_seed) 독립성 확보:** 시드 값이 IOCTL 코드를 대체하거나 오버라이드하여 발생하던 통제 불능 현상을 해결하기 위해 TARGET_IOCTLS 독립 설정을 도입. 이를 통해 취약점 재현(0x222003)과 미정의 코드 주입(0x222004)을 명확히 분리하여 참/거짓 양방향 음성(Negative) 케이스 정밀 검증 지원.

**가변 페이로드 크기 동적 엔진 도입 (FUZZ_MIN_SIZE / FUZZ_MAX_SIZE):** 단일 고정 크기 페이로드로 인해 매 반복 커널 버퍼를 초과하여 발생하던 착시 크래시를 방지하고자 MutatorEngine::NextSize() 구현. 매 반복 페이로드 크기를 1B~8192B 범위에서 가변 선택하여 커널 스택 버퍼 오버플로우 임계값 전후의 양성/음성 반응 관측.

<br>

**2) 구조적 랜덤 IOCTL 탐색 및 코퍼스 편향 최적화**

**코퍼스 기반 구조적 랜덤 탐색 모드 (IOCTL_MODE=random):** 알려진 유효 IOCTL 코퍼스(HEVD_IOCTL_CORPUS) 재사용 및 CTL_CODE 레이아웃 유지 기반의 인접 공간 탐색을 시드 결정론적으로 수행하여 탐색 폭을 대폭 확장.

코퍼스 배치 순서 조정을 통한 탐색 교착 해제: 배열 0번 인덱스에 위치했던 0x222003으로 인해 1회 차(iter 1) 진입 즉시 BSOD가 터져 후속 탐색이 중단되던 편향 현상을 해결하고자 코퍼스 배열 순서를 최적화.

<br>

**3) 파일 기반 IPC 완전 폐기 및 순수 TCP 소켓 스트리밍 완성**

**HGFS 및 게스트 디스크 파이프라인 전면 철거:** HGFS 쓰기 차단, flush 비내구성, VM Revert 시 파일 소실로 발생하던 유죄 시드 수거 누락 문제를 근본적으로 해결하기 위해 파일 기반 상태 저장 파이프라인 완전 파기.

**실시간 메모리 소켓 스트리밍 (TcpReporter):** 워커가 DeviceIoControl 커널 진입 직전 304바이트 고정 POD 레코드(KTCP 헤더)를 호스트 RAM으로 직접 송출. 커널 패닉으로 소켓 절단 시 호스트가 최후의 InIoctl 패킷을 지체 없이 유죄 시드로 즉시 확정/승격.
