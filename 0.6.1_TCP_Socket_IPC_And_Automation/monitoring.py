...
SNAPSHOT_NAME = cfg("SNAPSHOT_NAME", "set_host")
...
class ReportServer:
    ...
    def _persist_candidate(self, pkt, rank, total, age_ms):
        ...
        log(f"[SAVED:BSOD-CAND {rank}/{total}] {os.path.basename(bin_path)} | "
            f"worker {pkt['worker_id']} IOCTL 0x{pkt['ioctl']:X} seed 0x{pkt['seed']:08X} "
            f"age {age_ms}ms iter {pkt['iteration']}")
        return bin_path
...
# monitoring.py 내 run_fuzzer_in_vm 함수 수정안
    cmd = [
        VMRUN_PATH, "-vp", VM_ACCESS_PASS, "-T", "ws",
        "-gu", VM_USER, "-gp", VM_PASS,
...
def main():
    ...
    try:
        while True:
            ...
            #   실패 시 이번 라운드를 건너뛰되 커서(base_seed)는 전진시키지 않아,
            #   동일 시드로 다음 라운드에 다시 시도한다. (백오프 후 continue로 tight-loop 방지)
            if not revert_vm_with_retry():
                log("[VM] Skipping this iteration due to VM revert failure.")
                time.sleep(VM_RETRY_BACKOFF)
                continue

            # [Fix: P0] 라운드 본문 전체를 try/except로 감싸 예기치 못한 예외(vmrun 서브 단계,
            #   네트워크, 파일 IO 등)로 컨트롤러 프로세스가 종료되지 않도록 방어한다.
            #   KeyboardInterrupt는 BaseException이라 여기서 잡히지 않고 바깥 핸들러로 전파된다.
            try:
                wait_for_tools()
                ...
                if stats['total'] == 0:
                    ...
                    # [Fix: P1] 단일 유죄 승격과 별개로, 프리즈 직전 InIoctl 중이던 워커들을
                    #   공동 후보로 함께 보존한다(레이스로 인한 오귀속 대비). 이미 저장된 primary는
                    #   _saved_keys 중복 회피로 자동 스킵되어 중복 파일이 생기지 않는다.
                ...
                #   커서는 전진시키지 않아 실패한 시드를 다음 라운드에서 재시도한다.
