			...
			while (true) {
				...
				for (size_t i = 0; i < workers_.size(); ++i) {
					...
					if (worker.Wait(0)) {
						...
						if (exitCode != 0){
							// [수정]
							globalRapidDeathCount++; //Wait(0) 조건문 바로 밑에 위치하여 정상 종료 시에도 무조건 카운트 증가를 했었음.
							if (globalRapidDeathCount >= 10) {
								std::wcout << L"\n[FATAL ERROR] Worker spawn storm detected!\n"
									<< L"[!] Aborting master process immediately.\n";
								for (auto& w : workers_) w.Kill(1);
								return;
							}
						}
					...
					[삭제]
						...
						if (worker.IsRunning()) {
							hangCounters[i]++;
							if (hangCounters[i] >= 600) { // 600 x 50 = 30000 (30초)
								std::wcout << std::format(L"[!] Warning: Worker {} is HANGING. Terminating...\n", i);
								worker.Kill(1);
							}
						}
					}
					*/
					//[수정 후]
					/*
					수정 전: 워커가 700만 회를 열심히 돌리고 있어도 30초만 지나면 무조건 강제 사살.

					수정 후: 워커가 10분이 걸리든 1시간이 걸리든 퍼징을 진행 중(CPU 소비 중)이면 절대 죽이지 않음. 커널 데드락에 빠져 CPU 시간이 30초 동안 단 1밀리초도 안 움직일 때만 진짜 Hang으로 판정해 사살.
					*/
					else {
						if (worker.IsRunning()) {
							...
							if (GetProcessTimes(worker.GetProcessHandle(), &createTime, &exitTime, &kernelTime, &userTime)) { //worker.GetProcessHandle()은 WorkerProcess 클래스에 있는 hProcess_ 핸들을 반환하는 게터 함수입니다.
								...
								if (totalCpu > lastCpuTimes[i]) {
									...
									hangCounters[i]++;
									if (hangCounters[i] >= 600) { // 30초 동안 CPU 진척이 0이면 진짜 Hang
										std::wcout << std::format(L"[!] Warning: Worker {} is HANGING. Terminating...\n", i);
										worker.Kill(1);
									}
								}
