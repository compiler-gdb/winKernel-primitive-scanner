...
export namespace WinKernel::Driver{
	...
	private:
		...
		DWORD lastError_{ 0 }; // [Diag: rc=1 원인 국소화] CreateFileW 실패 시 GetLastError 보존
	...
	public:
		...
		explicit DriverController(const std::wstring& symLinkName) 
			...
			// [Diag: 드라이버 오픈 실패 코드 포착] 이후 API 호출로 GetLastError가 덮이기 전에 즉시 저장
			lastError_ = (hDevice_ == INVALID_HANDLE_VALUE) ? GetLastError() : 0;
		}
		...
		// [Diag: 진단용] 드라이버 오픈 실패 원인 (2=NOT_FOUND/미로드, 5=ACCESS_DENIED/권한부족 등). 성공 시 0.
			...
			*/
			...
			// ERROR_IO_PENDING: 비동기 작업이 정상적으로 커널 대기열에 접수되어 백그라운드에서 처리 중임을 의미하는 윈도우 정식 상태코드(997)입니다.
				...
				if (waitResult == WAIT_TIMEOUT) {	// WAIT_TIMEOUT: WaitForSingleObject에 설정한 시간이 다 돌 때까지 알람 벨이 울리지 않았을을 의미하는 Windows API의 약속된 상수 값입니다.
					...
					// 치명적 버그: 비동기 I/O 취소 후 스택 OVERLAPPED/이벤트를 즉시 해제하면
					// 커널이 취소 IRP를 처리하며 이미 해제된 스택 메모리에 접근해 손상이 발생.
					// [수정] 취소 IRP 처리가 완전히 끝날 때까지 동기 대기(Drain)한 뒤 이벤트 해제.
					GetOverlappedResult(hDevice_, &ov, &bytesReturned, TRUE);
